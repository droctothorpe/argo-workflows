...
func newSQLDBCache(namespace string, name string, sp *sqldb.SessionProxy, tableName string) MemoizationCache {
	return &sqlDBCache{
		namespace:    namespace,
		name:         name,
		sessionProxy: sp,
...
func (c *sqlDBCache) Load(ctx context.Context, key string) (*Entry, error) {
	...
	return &Entry{
		...
		LastHitTimestamp:  metav1.Time{Time: record.LastHitAt},
	}, nil
}

func (c *sqlDBCache) Save(ctx context.Context, key string, nodeID string, value *wfv1.Outputs) error {
	if !cacheKeyRegex.MatchString(key) {
